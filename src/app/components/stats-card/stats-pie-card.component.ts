import { Component, Input, OnChanges, AfterViewInit, OnDestroy, ElementRef, ViewChild, SimpleChanges } from '@angular/core';
...
export class StatsPieCardComponent implements OnChanges, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void { this.createOrUpdateChart(); }
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['data'] && !changes['data'].firstChange) this.createOrUpdateChart();
  }
  ngOnDestroy(): void { this.chart?.destroy(); }

  private getTop5(arr: ClusterStats[]): ClusterStats[] {
    return [...(arr || [])].sort((a, b) => (b.total_pontos || 0) - (a.total_pontos || 0)).slice(0, 5);
  }
  ...
  private createOrUpdateChart(): void {
    ...
    const labels: string[] = top.map(c => {
      const [lat, lng] = c.centro || [0, 0];
      return `${(lat ?? 0).toFixed(5)}, ${(lng ?? 0).toFixed(5)}`;
    });
    const values: number[] = top.map(c => c.total_pontos || 0);
          ...
          legend: {
            ...
            labels: {
              generateLabels: (chart) => {
                ...
                return items.map((it: any, i: number) => ({ ...it, text: `${labs[i]} — qtd: ${ds[i] ?? 0}` }));
              }
            }
          },
          tooltip: {
    ...
    if (this.chart) {
      ...
      this.chart = new Chart<'pie', number[], string>(this.pieCanvas.nativeElement, cfg);
    }
  }
}
